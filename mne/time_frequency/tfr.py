...
class EpochTFR(object):
    ...
    def __init__(self, data, freqs, times, events, event_id,
                 info, method=None):
        if data.ndim != 4:
    ...
    def subset_freq(self, fmin, fmax):
        ...
        epoch_info['description'] = "{'kind': 'TFRFreqSubset', 'freqs':"\
                                    " '{0}-{1}'.format(fmin, fmax)}"
    ...
    def subset_epoch(self, epoch_ixs):
        ...
        epoch_info = self.info.copy()
        return mne.time_frequency.AverageTFR(epoch_info, data_epoch,
                                             self.times, self.freqs,
                                             len(epoch_ixs),
                                             comment='TFREpochSubset',
                                             method=self.method)
    ...
    def write_hdf(self, fname, **kwargs):
        ...
        **kwargs : dict
        ...
        attrs = ['data', 'freqs', 'times', 'events', 'info', 'event_id']
        write_dict = {attr: getattr(self, attr) for attr in attrs}
